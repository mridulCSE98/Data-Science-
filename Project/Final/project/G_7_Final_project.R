...
article_patterns <- c("/news/", "/politics/news/", "/national/news/", "/banglanews-special/news/", "/opinion/news/")
...
cat("Saved", nrow(links_data), "article links from banglanews24.com homepage\n")
...
data <- read_csv(file.path(output_dir, "articles_with_text.csv"), locale = locale(encoding = "UTF-8"), show_col_types = FALSE)
texts <- data$article_text

bangla_stopwords <- c(
  "া", "ও","করে","এবং","না","থেকে","তিনি","করা","ন","েছে","একটি","বাংলাদেশ","হবে","ে","এক","ই","সঙ্গে",
  "বাংলাদেশের","নিয়ে","পর","রহমান","করতে","মে","কোনো","র","দেশের","আমাদের","যে","পারে","মধ্যে","হিবে","টাকা",
  "এর","এবং","না","থেকে","তিনি","করা","এ","হয়","তার","বলেন","কিন্তু", "তবে", "হয়েছে", "ছিল", "যা", "সে",
  "এই", "হয়", "জন্য", "না", "ওরে", "তুমি", "আমি", "তারা", "আমরা", "আপনি", "তাহলে", "করেন", "হয়েছে", "ড"
)

clean_text <- function(text) {
  text %>%
    str_replace_all("[^\\p{Bengali}\\s]", " ") %>%
    str_replace_all("[\\p{P}]", " ") %>%
    str_replace_all("[\u09E6-\u09EF]", " ") %>%
...
remove_stopwords <- function(text) {
  ...
  filtered <- words[!words %in% bangla_stopwords]
